...
export default {
  content: ["./demo/**/*.{html,js,jsx,md,mdx,ts,tsx}"],
  theme: {
    extend: {
      fontFamily: {
  ...
  plugins: [
    ...
    plugin(function fontVariations({ addComponents, matchUtilities, theme }) {
        const fontVariationProps = Object.entries(theme('fontVariationSettings')).map(([key, values]) => {
          /**
          ...
          const cssPropValue = (key, value) =>
            key === 'stretch' ? `${value}%` // .font-stretch
            : key === 'italic' ? `oblique ${value}deg` // .font-oblique
            : value; // font-weight
          return values.map(value => [`.font-${key}-${value}`, { [`font-${key.replace(/italic/g, 'style')}`]: cssPropValue(key, value) }]);
        }).flat();
